...
class TextToSpeechService {
    ...
    async generateSmartSpeechText(text, languageCode = 'en') {
        ...
        if (this.smartTextCache.has(cacheKey)) {
            console.log('Using cached smart speech text');
            return this.smartTextCache.get(cacheKey);
        }

        try {
            console.log(`Generating smart speech for language: ${languageCode}`);
            ...
            if (data.success && data.data && data.data.smartText) {
                console.log('Smart speech generation successful');
                ...
                console.warn('Smart speech API returned unsuccessful response');
                return this.preprocessTextForSpeech(text, languageCode);
            }
        } catch (error) {
            console.warn('Failed to generate smart speech text, using fallback:', error.message);
    ...
    async speakSmart(text, languageCode = 'en', options = {}) {
        try {
            ...
            console.warn('Smart speech failed, falling back to regular speech:', error);
